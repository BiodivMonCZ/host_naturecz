load_vmb <- function(vmb_x = 1, clean = TRUE) {
  ...
  # VMB1 - prvni aktualizace mapovani ----  
  } else if(vmb_x == 2) {
    ...
    paseky_a1 <- 
      read.csv2(
        "S:/Složky uživatelů/Gaigr/hodnoceni_stanovist_grafy/paseky_a1_results_20240814.csv"
        )
    ...
    # VMB0 - aktualni mapovani ----  
  } else if(vmb_x == 0) {
    # VMB - VRSTVA AKTUÁLNÍ
    vmb_x_shp_akt <- 
      sf::st_read(
        "//bali.nature.cz/du/Mapovani/Biotopy/CR_AKTUALNI/X_Segment.shp"
        )
    vmb_x_dbf_akt <- 
      sf::st_read(
    ...
    vmb_pb_shp_akt <- 
      sf::st_read("//bali.nature.cz/du/Mapovani/Biotopy/CR_AKTUALNI/PB_Segment.shp", 
                  options = "ENCODING=WINDOWS-1250"
                  )
    vmb_pb_dbf_akt <- 
    ...
    paseky_23 <- 
      read.csv2(
        "S:/Složky uživatelů/Gaigr/hodnoceni_stanovist_grafy/paseky_results_20220927.csv"
        )
